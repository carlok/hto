        ...
        switch (request.headers['content-type']) {
            case 'application/json':
                if (parseInt(request.server.app.decrypt_input) === 0) {
                    ...
                    reply(Boom.badRequest('Wrong Content-Type (B)'));
                }
            ...
            case 'text/plain':
                if (parseInt(request.server.app.decrypt_input) === 1) {
                    ...
                    try {
                        a = JSON.parse(decrypt(String(request.payload)));
                    } catch (mError) {
                    ...
                    reply(Boom.badRequest('Wrong Configuration'));
                }
                ...
                break;
            default:
                reply(Boom.badRequest('Wrong Content-Type (A)'));
        }
    };
